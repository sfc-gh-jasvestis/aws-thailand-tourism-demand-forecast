...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Arrivals Forecast (30d)" value={kpiVal('Arrivals Forecast (30d)', '3.2M')} status="neutral" />
        <KPICard title="Revenue Forecast" value={kpiVal('Revenue Forecast', '฿148B')} status="neutral" />
        <KPICard title="Forecast Accuracy" value={kpiVal('Forecast Accuracy', '94.1%')} status="neutral" />
        <KPICard title="Markets Tracked" value={kpiVal('Markets Tracked', '42')} status="neutral" />
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="lg:col-span-1">
          <GeoMap
            country="thailand"
            labels={{ entity: 'Markets', event: 'Search Signals', alert: 'Forecast Misses' }}
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'Arrivals (K)' }]}
          title="Tourist Arrivals (Monthly)"
        />
        <Chart
          ...
          title="Revenue by Source Market"
        />
      </div>
        </div>
      </div>
      <DataTable
        columns={[
          ...
          { key: 'm1', header: 'Arrivals (K)' },
          { key: 'm2', header: 'Revenue Forecast' },
          { key: 'm3', header: 'Forecast Accuracy' },
          { key: 'events', header: 'Search Signals' },
          { key: 'alerts', header: 'Forecast Misses' },
        ]}
        data={data?.entities || []}
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="China Recovery" value={kpiVal('China Recovery', '78%')} />
        <KPICard title="Avg Spend/Tourist" value={kpiVal('Avg Spend/Tourist', '฿47K')} />
        <KPICard title="Visa-Free Impact" value={kpiVal('Visa-Free Impact', '+22%')} />
      </div>
      <Chart
  ...
  const domainTab2 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        />
        <ActionMemo
          persona={{ name: 'Thanakorn Wanglee', role: 'TAT Deputy Governor' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Alert Phuket: Chinese groups +40% next month', 'Recommend flight capacity increase BKK-Mumbai', 'Brief immigration on Songkran surge'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'What is the 90-day forecast for Chinese arrivals?',
          'Which destinations are under-capacity for high season?',
          'Show flight capacity vs arrivals correlation',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Capacity Planning', icon: '⚡', content: domainTab2 },
